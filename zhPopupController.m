...
@property (nonatomic, assign) BOOL isImpactNeeded;
@end
...
- (void)presentDuration:(NSTimeInterval)duration
    ...
    if (self.keyboardChangeFollowed && self.becomeFirstResponded) {
        ...
        if (isBounced) {
            ...
            //animateWithDuration 为0，动画不一定就没了，options为7<<16就还会有动画
            [UIView animateWithDuration:duration delay:delay options:options animations:^{
                self.maskView.alpha = 1;
                [self finalSlideStyle];
                self.view.center =  [self finalCenter];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    ...
    if (self.gestureRecognizerShouldReceiveTouchBlock) {
        return self.gestureRecognizerShouldReceiveTouchBlock(gestureRecognizer, touch, self);
    }
    return YES;
}
...
- (void)handlePan:(UIPanGestureRecognizer *)g {
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            if (self.impactNeeded) {
                BOOL isImpactNeeded = NO;
                CGFloat pullPercent = 0.0;
                switch (self.layoutType) {
                    case zhPopupLayoutTypeTop: {
                        isImpactNeeded =
                        - self.view.frame.origin.y > self.view.frame.size.height * self.panDismissRatio;
                        pullPercent = (- self.view.frame.origin.y)/(self.view.frame.size.height * self.panDismissRatio);
                    } break;
                    case zhPopupLayoutTypeLeft: {
                        isImpactNeeded = - self.view.frame.origin.x > self.view.bounds.size.width * self.panDismissRatio;
                        pullPercent = (- self.view.frame.origin.x)/(self.view.bounds.size.width * self.panDismissRatio);
                    } break;
                    case zhPopupLayoutTypeBottom: {
                        isImpactNeeded =
                        (self.maskView.bounds.size.height - self.view.frame.origin.y) < self.view.frame.size.height * (1 - self.panDismissRatio);
                        pullPercent = (self.view.frame.size.height - (self.maskView.bounds.size.height - self.view.frame.origin.y))/(self.view.frame.size.height * self.panDismissRatio);
                    } break;
                    case zhPopupLayoutTypeRight: {
                        isImpactNeeded = (self.maskView.bounds.size.width - self.view.frame.origin.x) < self.view.frame.size.width * (1 - self.panDismissRatio);
                        pullPercent = (self.view.frame.size.width - (self.maskView.bounds.size.width - self.view.frame.origin.x))/(self.view.frame.size.width * self.panDismissRatio);
                    } break;
                    case zhPopupLayoutTypeCenter: {
                        if (_directionalVertical) {
                            isImpactNeeded = CGRectGetMinY(g.view.frame) > self.maskView.bounds.size.height * self.panDismissRatio;
                ...
                if (self.canDissmissBlock) {
                    self.canDissmissBlock(self, pullPercent, (_lastImpact != isImpactNeeded)&&isImpactNeeded);
                }
                
                if (_lastImpact != isImpactNeeded) {
                                        
                    if (self.impactNeeded) {
//                        if (UserDefaults.needImpact) {
//                            impactFeedback();
//                        }
                    }
                    _lastImpact = isImpactNeeded;
                }
                
//                self.isImpactNeeded = isImpactNeeded;
            }
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (isDismissNeeded) {
                ...
                if (self.defaultPopupBlock) {
                    self.defaultPopupBlock(self);
                }
...
- (void)zh_presentPopupController:(zhPopupController *)popupController duration:(NSTimeInterval)duration bounced:(BOOL)isBounced completion:(void (^)(void))completion {
    return [self zh_presentPopupController:popupController duration:duration delay:0 options:duration?7<<16:0 bounced:isBounced completion:completion];
}
...
- (void)zh_dissmissPopupController:(zhPopupController *)popupController duration:(NSTimeInterval)duration completion:(void (^)(void))completion {
    return [self zh_dissmissPopupController:popupController duration:duration delay:0 options:duration?7<<16:0 completion:completion];
}
...
- (void)showDuration:(NSTimeInterval)duration completion:(void (^ __nullable)(void))completion {
    return [self.keyWindow zh_presentPopupController:self duration:duration completion:completion];
}
...
- (void)dismissWithDuration:(NSTimeInterval)duration completion:(void (^)(void))completion {
    return [self.proxyView zh_dissmissPopupController:self duration:duration completion:completion];
}
